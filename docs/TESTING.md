...
- `npm run test:preferences`: native GTK/libadwaita controls and GSettings with
  ...
  settings, and closing. Use `dbus-run-session -- xvfb-run -a` without a display.

### Verified in the development container (2026-09-07)

23 Node tests, strict schema compilation, archive construction/integrity,
real GJS/Soup integration, and native preferences tests passed. GJS 1.82.3 and
libadwaita 1.7.6 were extracted into a temporary directory; no system desktop
installation was made. Headless GTK produced environment warnings about the
missing accessibility bus and portal services, but the UI assertions passed.
