...
export class SoupTransport {
    constructor({userAgent = 'ntfy-gnome'} = {}) {
        this._session = new Soup.Session({
            user_agent: userAgent,
            // Socket I/O timeout; covers connect and TLS. Silent streams are
            // abandoned earlier by the client's idle watchdog.
            timeout: 90,
            max_conns: 24,
    ...
    open(url, token = null) {
        ...
        // Give every subscription its own TCP connection. With HTTP/2 both
        // streams share one connection and a silently dropped connection left
        // the client waiting forever without any error (libsoup 3.6).
